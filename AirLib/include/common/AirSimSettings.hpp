...
#include <Source/MultiDroneGameInstance.h>
...
public: //types
	...
	struct BatteryConsumptionSetting {
        BatteryConsumptionSetting() {};
        BatteryConsumptionSetting(std::string c_name, float c_power) { component_name = c_name; consumption_power = c_power; }
		std::string component_name;
        float consumption_power;
	};
    ...
    struct VehicleSetting {
		...
		std::map<std::string, BatteryConsumptionSetting> battery_consumptions;
...
public: //methods
    ...
    void load(std::function<std::string(void)> simmode_getter, bool using_unreal_physics)
    {
        ...
        loadCoreSimModeSettings(settings_json, simmode_getter, using_unreal_physics);
        ...
        loadPawnPaths(settings_json, pawn_paths, using_unreal_physics);
...
private:
    ...
    void loadCoreSimModeSettings(const Settings& settings_json, std::function<std::string(void)> simmode_getter, bool using_unreal_physics)
    {
		...
		if (using_unreal_physics)
			physics_engine_name = "UnrealPhysicsEngine";
		else
			physics_engine_name = "FastPhysicsEngine"; //this value is only informational for now

		//initializePawnPaths(pawn_paths, using_unreal_physics);
    }
    ...
    static std::unique_ptr<VehicleSetting> createMavLinkVehicleSetting(const Settings& settings_json)
    {
        ...
        //connection_info.relay_server = settings_json.getBool("RelayServer", connection_info.relay_server);
        ...
        const std::string vehicle_name)
    {
		...
		loadBatterySettings(settings_json, "Battery", vehicle_setting->battery, vehicle_setting->battery_consumptions);
    ...
    static void initializeVehicleSettings(const std::string &simmode_name, std::map<std::string, std::unique_ptr<VehicleSetting>>& vehicles)
    {
        ...
        if (simmode_name == kSimModeTypeMultirotor) {
        	...
        	simple_flight_setting->vehicle_name = std::to_string(num);
            ...
            //physx_car_setting->vehicle_name = "PhysXCar";
       		physx_car_setting->vehicle_name = std::to_string(num);
...
public:
	static void initializePawnPaths(std::map<std::string, PawnPath>& pawn_paths, int vehicleType)
    {
        pawn_paths.clear();
        ...
        {
        case 0:
            path = "Class'/AirSim/Blueprints/BP_FlyingPawn.BP_FlyingPawn_C'";
            break;
        case 1:
            path = "Class'/AirSim/Blueprints/BP_FlyingPawnPhysX.BP_FlyingPawnPhysX_C'";
            break;
        case 2:
            path = "Class'/Game/Blueprints/Multirotor/BP_FlyingPawn_ETRI.BP_FlyingPawn_ETRI_C'";
            break;
        default:
            path = "Class'/AirSim/Blueprints/BP_FlyingPawn.BP_FlyingPawn_C'";
            break;
	...
	static void loadPawnPaths(const Settings& settings_json, std::map<std::string, PawnPath>& pawn_paths, bool using_unreal_physics = false)
    {
        auto raw_inst = GWorld->GetGameInstance();
        if (raw_inst == nullptr)
            return;
        auto inst = (UMultiDroneGameInstance*)raw_inst;
        auto type = TCHAR_TO_ANSI(*(inst->Setting.VehicleType));
		initializePawnPaths(pawn_paths, std::stoi(type));
...
private:
    ...
    void loadOtherSettings(const Settings& settings_json)
    {
        ...
        use_propeller_wind = settings_json.getBool("UsePropellerWind", true);
	...
	static void loadBatterySettings(const Settings& settings_json, const std::string& collectionName,
		BatterySetting& battery, std::map<std::string, BatteryConsumptionSetting>& battery_consumptions)
	{
		msr::airlib::Settings battery_settings;
		if (settings_json.getChild(collectionName, battery_settings)) 
        {
            battery.enabled = battery_settings.getBool("Enabled", false);
            if (!battery.enabled)
                return;
            
			battery.volt = battery_settings.getFloat("Volt", 1.f);
            battery.mAh = battery_settings.getFloat("mAh", 1.f);
            battery.capacity_rate = battery_settings.getFloat("CapacityRate", 1.f);

			msr::airlib::Settings battery_comsumptions_settings;
            if (battery_settings.getChild("Components", battery_comsumptions_settings)) 
			{
				std::vector<std::string> keys;
				battery_comsumptions_settings.getChildNames(keys);
				for (const auto& key : keys)
				{
					battery_consumptions[key] = BatteryConsumptionSetting(key, battery_comsumptions_settings.getFloat(key, 0.f));                    
                }

				//msr::airlib::Settings child;
				//battery_settings.getChild(key, child);
