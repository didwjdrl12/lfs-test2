...
//#include <Source/AirLib/deps/MavLinkCom/include/MavLinkMessages.hpp>
#include "MavLinkMessages.hpp"
...
private: //methods
    ...
    {
        //createCPSConnection(connection_info);

        /*if(connection_info.relay_server)
            createCPSConnection(connection_info);
        else */
    ...
    void createCPSConnection(const AirSimSettings::MavLinkConnectionInfo& connection_info)
    {
        close();
        ...
        //if (connection_info.relay_server)
        {
            auto msg = Utils::stringf("Waiting for Relay connection");
            addStatusMessage(msg);
                        
            connection_ = std::make_shared<mavlinkcom::MavLinkConnection>();
            connection_ = mavlinkcom::MavLinkConnection::connectLocalUdp("CPS", "192.168.0.243", 14550);
            //mav_vehicle_->connect(connection_);

            //std::shared_ptr<UdpClientPort> socket = std::make_shared<UdpClientPort>();
            //socket->connect("192.168.0.243", 14550, "192.168.0.243", 14580);
            
            //connection_->startListening("CPS", socket);

            connection_->subscribe([=](std::shared_ptr<mavlinkcom::MavLinkConnection> connection, const mavlinkcom::MavLinkMessage& msg) {
                unused(connection);
                //processMavMessages(id);

                addStatusMessage(std::to_string(msg.msgid));
                ...
                if (mav_vehicle_ == nullptr)
                {
                    if (msg.msgid == HeartbeatMessage.msgid)
                    {
                        ...
                        mav_vehicle_->connect(connection);
                        connectVehicle();
                    }
                }
                else
                {
                    processQgcMessages(msg);
                    if (msg.msgid == mavlinkcom::MavLinkGlobalPositionInt::kMessageId)
                    {
                        mavlinkcom::MavLinkGlobalPositionInt pos;
                        pos.decode(msg);
                        cnt++;

                        GeoPoint geo;
                        geo.latitude = pos.lat * 0.0000001f;
                        geo.longitude = pos.lon * 0.0000001f;
                        geo.altitude = pos.alt * 0.001f;

                        auto ned = msr::airlib::EarthUtils::GeodeticToNedFast(geo, AirSimSettings::singleton().origin_geopoint.home_geo_point);

                        //YawMode yaw;
                        //yaw.setZeroRate();
                        //commandPosition(ned.x(), ned.y(), ned.z(), yaw);
                        //mav_vehicle_->moveToLocalPosition(ned.x(), ned.y(), ned.z(), false, 0.f);
                    }
                    else if(msg.msgid == mavlinkcom::MavLinkServoOutputRaw::kMessageId)
                    {
                        /*mavlinkcom::MavLinkServoOutputRaw rotors;
                        rotors.decode(msg);
                        rotor_controls_[0] = rotors.servo1_raw;
                        rotor_controls_[1] = rotors.servo2_raw;
                        rotor_controls_[2] = rotors.servo3_raw;
                        rotor_controls_[3] = rotors.servo4_raw;*/
                    }
                }
            });
