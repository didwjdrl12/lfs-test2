...
std::shared_ptr<MavLinkConnection>  MavLinkConnectionImpl::createConnectionForCPS(const std::string& nodeName, std::shared_ptr<UdpClientPort> port)
{
    // std::shared_ptr<MavLinkCom> owner, const std::string& nodeName
    std::shared_ptr<MavLinkConnection> con = std::make_shared<MavLinkConnection>();
    con->startListeningForCPS(nodeName, port);
    return con;
}
...
std::shared_ptr<MavLinkConnection>  MavLinkConnectionImpl::connectCPS(const std::string& nodeName, const std::string& localAddr, int localPort, const std::string& remoteAddr, int remotePort)
{
    std::string local = localAddr;
    if (remoteAddr == "127.0.0.1") {
        local = "127.0.0.1";
    ...
    socket->connectCPS(local, localPort, remoteAddr, remotePort);

    return createConnectionForCPS(nodeName, socket);
}
...
void MavLinkConnectionImpl::startListeningForCPS(std::shared_ptr<MavLinkConnection> parent, const std::string& nodeName, std::shared_ptr<UdpClientPort> connectedPort)
{
    name = nodeName;
    con_ = parent;
    if (CPSport != connectedPort) {
        close();
        CPSport = connectedPort;
    }
    closed = false;

    Utils::cleanupThread(read_thread);
    read_thread = std::thread{ &MavLinkConnectionImpl::readPacketsForCPS, this };
...
void MavLinkConnectionImpl::readPacketsForCPS()
{
    //CurrentThread::setMaximumPriority();
    CurrentThread::setThreadName("MavLinkThread");
    std::shared_ptr<UdpClientPort> safePort = this->CPSport;
    ...
    while (hr == 0 && con_ != nullptr && !closed)
    {
        ...
        int count = safePort->readForCPS(buffer, MAXBUFFER);
        if (count <= 0) {
