...
public:
	...
	int connectCPS(const std::string& localHost, int localPort, const std::string& remoteHost, int remotePort)
	{
	...
	int readForCPS(uint8_t* result, int bytesToRead)
	{
		int bytesRead = 0;
		...
		while (!closed_)
		{
			...
			socklen_t addrlen = sizeof(sockaddr_in);
			int rc = recvfrom(sock, reinterpret_cast<char*>(result), bytesToRead, 0, reinterpret_cast<sockaddr*>(&remoteaddr), &addrlen);
			if (rc < 0)
			{
				int hr = checkerror();
#ifdef _WIN32
...
void UdpClientPort::connectCPS(const std::string& localHost, int localPort, const std::string& remoteHost, int remotePort)
{
	impl_->connectCPS(localHost, localPort, remoteHost, remotePort);
}
...
int UdpClientPort::readForCPS(uint8_t* buffer, int bytesToRead)
{
	return impl_->readForCPS(buffer, bytesToRead);
}
