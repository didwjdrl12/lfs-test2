...
namespace J2y.Network
{
	...
	public static class NetBitWriter
	{
		...
		public static int ReadInt32(byte[] fromBuffer, int numberOfBits, int readBitOffset)
		{
			NetException.Assert(((numberOfBits > 0) && (numberOfBits <= 32)), "ReadInt32() can only read between 1 and 32 bits");
#endif
			int returnValue;
			if (numberOfBits <= 8)
			{
		...
		/// </summary>
		//[CLSCompliant(false)]
		public static int WriteInt32(int source, int numberOfBits, byte[] destination, int destinationBitOffset)
		{
#if BIGENDIAN
			// reorder bytes
